...
2.  **Group Sparsity Projection (BST):** For each signal family $g$, apply block soft-thresholding:
    $$\mathcal{A}_g^{(k+2/3)} = \mathcal{A}_g^{(k+1/3)} \max\left(1 - \frac{\eta \lambda_{\text{grp}}}{\|\mathcal{A}_g^{(k+1/3)}\|_F}, 0\right)$$
3.  **Rank-Sparsity Projection (SVT):** Compute SVD of the intermediate operator $\mathcal{A}^{(k+2/3)} = \mathbf{U}\mathbf{S}\mathbf{V}^T$ and threshold:
